...
/*	var	margin = {top: 30, right: 20, bottom: 30, left: 50},
	...
	var	xAxis = d3.svg.axis().scale(x)
		.orient("bottom").tickFormat(d3.time.format("%m-%dT%H"));
	...
	// Adds the svg canvas
	var	svg = d3.select("#graph-lines2")
		.append("svg")
	...
	y.domain([0, d3.max(data, function(d) { return Math.max(d.predicted); })]);
...
function drawTrend(resultJsons) {
	...
	var xAxis = d3.svg.axis().scale(x).orient("bottom").tickFormat(d3.time.format("%m-%dT%H"));
	...
	var svg = d3.select("#graph-lines2").append("svg").attr("width",
